...
<aside class="w-64 bg-white  flex flex-col">
    ...
    <nav class="flex-1 px-4 py-6 space-y-1 text-gray-700 text-sm font-medium">
        ...
        @if (Auth::user()->role === 'user')
            ...
            <a href="{{ route('mood') }}"
                class="flex items-center gap-3 px-3 py-2 rounded-lg {{ request()->routeIs('mood') ? 'bg-teal-50 text-teal-700 font-semibold border-l-4 border-teal-500' : 'hover:bg-gray-50 hover:text-teal-600' }}">
                <i data-lucide="smile" class="w-5 h-5"></i> Mood Tracker
            </a>
            <a href="{{ route('journal') }}"
                class="flex items-center gap-3 px-3 py-2 rounded-lg {{ request()->routeIs('journal') ? 'bg-teal-50 text-teal-700 font-semibold border-l-4 border-teal-500' : 'hover:bg-gray-50 hover:text-teal-600' }}">
                <i data-lucide="book-open" class="w-5 h-5"></i> Jurnal
            </a>
            <a href="{{ route('chatbot') }}"
                class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-50 hover:text-teal-600 {{ request()->routeIs('chatbot') ? 'bg-teal-50 text-teal-700 font-semibold border-l-4 border-teal-500' : 'hover:bg-gray-50 hover:text-teal-600' }}">
                <i data-lucide="message-circle" class="w-5 h-5"></i> Chatbot AI
            </a>
            <a href="{{ route('recomendation') }}"
